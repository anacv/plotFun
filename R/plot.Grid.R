...
#' 
#' @param data: mattix or list of matrices to be plotted. When it is a list, a multiplot is generated (each element of the list is plotted in a subplot).
...
plot.Grid <- function(data, lon, lat, lonLims=c(min(lon),max(lon)), latLims=c(min(lat),max(lat)), lattice=NULL, breaks= NULL, palette=rainbow(10), window.width=8, window.height=6, pch.marker=16, title.main=NULL, title.single=NULL,  unit.text=NULL, cex.marker=1, cex.main=1,  cex.single=1, cex.unit=1, cex.textcbar=1, width.cbar=0.1, disp.warnings=FALSE, export=FALSE, export.format=NULL, export.path=NULL, xlab=NULL, ylab=NULL, lon.points=NA, lat.points=NA){
	...
	if(is.matrix(data)){
		...
		# Plot subplots
		for (i in 1:n){
			plot.Grid1(data[[i]], lon, lat, lonLims, latLims, breaks, palette, cex.single, title.single[i], xlab, ylab, lon.points=lon.points, lat.points=lat.points)
		}
...
plot.Grid1 <- function(x, lon, lat, lonLims,latLims,  breaks= breaks, palette=palette, cex.main, title.main, xlab, ylab, lon.points=lon.points, lat.points=lat.points) {
...
plot.colorbar <- function(breaks, palette=palette, unit.text, cex.unit, cex.textcbar) {
	...
	if(length(breaks)>100){
		axis(4,at=seq(1,length(breaks),100),lab=breaks[seq(1,length(breaks),100)] , cex.axis=cex.textcbar)
	}else{
